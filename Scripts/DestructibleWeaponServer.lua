...
-- This works for weapons that don't have modified damage like Advanced weapons
-- local damageAmountToPlayers = WEAPON.directDamage

local damageAmountToPlayers = 0 -- temp setting

function OnTargetImpact(theWeapon, impactData)
	local amount = damageAmountToObjects
	if impactData.targetObject ~= nil and impactData.targetObject:IsA("Player") then
...
function OnObjectDamaged(id, prevHealth, dmgAmount, impactPosition, impactRotation, sourceObject)
	if sourceObject == WEAPON.owner then
        ...
        -- STAT TRACKING
        WEAPON.owner:AddResource("NonPlayerDamage", dmgAmount)
...
function BroadcastDamageFeedback(amount, position)
	...
	-- KBTODO: Temp fix so we're not doing double damage with advanced weapons
	if Object.IsValid(player) and amount > 0 then
